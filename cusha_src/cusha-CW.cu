...
// GPU kernel performing vertex-centric computation having Concatenated Windows (CW) representation.
__global__ void CuSha_CW_GPU_kernel(	const uint num_of_shards,
										...
										const uint* shard_concatenated_windows_sizes,
	...
	uint shard_starting_address = shard_sizes [ blockIdx.x ];
	uint shard_ending_address = shard_sizes [ blockIdx.x + 1 ];
	...
	if( __syncthreads_or( flag ) ) {	// Requires (CC>=2.0).
		...
		uint shard_CW_starting_address = shard_concatenated_windows_sizes[ blockIdx.x ];
		uint shard_CW_ending_address = shard_concatenated_windows_sizes[ blockIdx.x + 1 ];

		for ( 	uint EntryAddress = shard_CW_starting_address + threadIdx.x;
				EntryAddress < shard_CW_ending_address;
				EntryAddress += blockDim.x	) {

				SrcValue [ Mapper [ EntryAddress ] ] = local_Vertices [ SrcIndex [ EntryAddress ] - shard_offset ];
...
bool processGraphCW(	CWGraph* Host_Graph,
	...
	// CW representation required arrays.
	unsigned int *dev_SrcIndex, *dev_DestIndex, *dev_Mapper;
	Vertex *dev_SrcValue, *dev_VertexValues;
	Edge *dev_EdgeValue;
	Vertex_static *dev_SrcValue_static;
	unsigned int *dev_shard_concatenated_windows_sizes, *dev_shard_sizes;
	...
	CUDAErrorCheck ( cudaMalloc( (void**)&dev_Mapper, Host_Graph->num_of_edges * sizeof(unsigned int) ) );
	...
	CUDAErrorCheck ( cudaMemcpyAsync( dev_shard_concatenated_windows_sizes, Host_Graph->shard_concatenated_windows_sizes, ( Host_Graph->num_of_shards + 1 )*sizeof(unsigned int), cudaMemcpyHostToDevice ) );
	...
	CUDAErrorCheck ( cudaMemcpyAsync( dev_Mapper, Host_Graph->mapper, (Host_Graph->num_of_edges)*sizeof(unsigned int), cudaMemcpyHostToDevice ) );
	...
	fprintf( stdout, "Processing graph using Concatenated Windows representation ...\n" );
	unsigned int counter = 0;
	setTime();
	do {
		finished = JOB_FINISHED;
		CUDAErrorCheck ( cudaMemcpyAsync ( dev_finished, &finished, sizeof(int), cudaMemcpyHostToDevice ) );
		CuSha_CW_GPU_kernel <<< gridDim, blockDim, requiredSharedMem >>> (	Host_Graph->num_of_shards,
																			blockSize_and_N.N,
																			dev_SrcIndex,
																			dev_Mapper,
																			dev_DestIndex,
																			...
																			dev_EdgeValue,
																			dev_SrcValue_static,
																			dev_shard_concatenated_windows_sizes,
																			dev_shard_sizes,
																			dev_finished);
	...
	CUDAErrorCheck( cudaFree( dev_Mapper ) );
	...
	CUDAErrorCheck( cudaFree( dev_shard_concatenated_windows_sizes ) );
	CUDAErrorCheck( cudaFree( dev_shard_sizes ) );

	return(EXIT_SUCCESS);
}

void ExecuteCuShaCW(	FILE* inputFile,
	...
	fprintf( stdout, "Organizing the graph to process it using Concatenated Windows representation ...\n");
	...
	unsigned int* windowOffsets = (unsigned int*)calloc((HostGraph.num_of_shards*HostGraph.num_of_shards+1), sizeof(unsigned int));
	assert( windowOffsets );
	...
	CUDAErrorCheck ( cudaHostAlloc( (void**) &(HostGraph.mapper), (HostGraph.num_of_edges) * sizeof(unsigned int), cudaHostAllocDefault ) );	// Additional mapper array.
	CUDAErrorCheck ( cudaHostAlloc( (void**) &(HostGraph.shard_concatenated_windows_sizes), (HostGraph.num_of_shards+1) * sizeof(unsigned int), cudaHostAllocDefault ) );

	// This part takes care of conversion of indices and filling up mapper for CW.
	HostGraph.shard_concatenated_windows_sizes[0] = 0;
	unsigned int incrementing_address = 0;
	for( int i = 0; i < HostGraph.num_of_shards; ++i ) {
		for( int j = 0; j < HostGraph.num_of_shards; ++j ) {
			unsigned int window_size = windowOffsets[(HostGraph.num_of_shards*j)+i+1] - windowOffsets[(HostGraph.num_of_shards*j)+i];

			memcpy( HostGraph.incoming_index + incrementing_address,
				temp_shards[j].incoming_index + windowOffsets[HostGraph.num_of_shards*j+i] - windowOffsets[HostGraph.num_of_shards*j],
				window_size * sizeof(unsigned int) );

			for( int k = 0; k < window_size; ++k) {
				HostGraph.mapper[ incrementing_address+k ] = windowOffsets[HostGraph.num_of_shards*j+i] + k;
			}

			incrementing_address += window_size;
		}
		HostGraph.shard_concatenated_windows_sizes[i+1] = incrementing_address;
	}
	...
	if ( processGraphCW(&HostGraph, blockSize_N) == EXIT_FAILURE) {
	...
	CUDAErrorCheck ( cudaFreeHost(HostGraph.shard_concatenated_windows_sizes) );
	CUDAErrorCheck ( cudaFreeHost(HostGraph.shard_sizes) );
	CUDAErrorCheck ( cudaFreeHost(HostGraph.incoming_index) );
	CUDAErrorCheck ( cudaFreeHost(HostGraph.mapper) );
